...
//        quickRef.put(uid, templateItem);
	...
	private void readTemplate(Identifiers identifiers){
        ...
        try {
            ...
            if(!templateContainsAllCompulsoryFields()){
                throw new Exception("Missing compulsory field(s) in template: "+compulsoryItems.stream().collect(Collectors.joining(" ")));
            }
        } catch (Exception e) {
	...
	private void checkCompulsoryItem(TemplateItem templateItem){
	    // compulsory items:
        templateItem.getPrintName();
    }
...
//			if(dataCol.equalsIgnoreCase("")) log.log(Level.SEVERE, "Please make sure the first item in your repeating group is not a None item");
	...
	private void checkCompulsory(TemplateItem templateItem){
	    int index = compulsoryItems.indexOf(templateItem.getPrintName().toLowerCase());
	    if(index>=0){
	        compulsoryItems.remove(index);
        }
    }

    private boolean templateContainsAllCompulsoryFields(){
	    return compulsoryItems.size()==0;
    }
    ...
    private List<String> compulsoryItems = new ArrayList<>(Arrays.asList("studysubjectid", "crfname", "crfversion"));
